  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 ">
        <div>
          <h1 className="text-3xl font-bold text-emerald-600 text-center">Welcome to Hierro A Medida</h1>
          <p className="text-center text-gray-600 mt-2">Let&apos;s set up your profile to get started</p>
        </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {LANGUAGES.map((language) => (
                  <FormField
                    ...
                    render={({ field }) => {
                      return (
                        <FormItem key={language.id} className="flex items-center space-x-2">
                          <FormControl>
                            <Checkbox
                          ...
                          <FormLabel className="font-normal cursor-pointer">{language.label}</FormLabel>
                        </FormItem>
                      )
                    }}
                  />
          ...
          <FormControl>
            <div>
              <input
                ...
                id="profile-photo"
              />
              <label htmlFor="profile-photo" className="block border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer hover:border-emerald-600 transition">
                {profilePreview ? (
                  <img src={profilePreview || "/placeholder.svg"} alt="Profile" className="w-32 h-32 object-cover mx-auto rounded" />
                ) : (
                  <div>
        ...
        <div className="flex gap-4 justify-end pt-6">
          <Button variant="outline" className="border-emerald-600 text-emerald-600 bg-transparent" disabled>
            Previous
          </Button>
